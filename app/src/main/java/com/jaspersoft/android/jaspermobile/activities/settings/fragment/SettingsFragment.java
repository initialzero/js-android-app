...
public class SettingsFragment extends BasePreferenceFragment {
    ...
    public void onCreate(final Bundle savedInstanceState) {
        ...
        connectTimeoutPref.setOnPreferenceChangeListener(new Preference.OnPreferenceChangeListener() {
            ...
            public boolean onPreferenceChange(Preference preference, Object newValue) {
                try {
                    long millisValue = toMillisValue(newValue);
                    mServerConfig.withConnectionTimeOut(millisValue, TimeUnit.MILLISECONDS);
        ...
        readTimeoutPref.setOnPreferenceChangeListener(new Preference.OnPreferenceChangeListener() {
            ...
            public boolean onPreferenceChange(Preference preference, Object newValue) {
                try {
                    long millisValue = toMillisValue(newValue);
                    mServerConfig.withReadTimeout(millisValue, TimeUnit.MILLISECONDS);
                    updateSummary(preference, newValue, R.string.st_summary_sec);
                    return true;
                } catch (NumberFormatException ex) {
                    Toast.makeText(getActivity(),
    ...
    private long toMillisValue(Object newValue) {
        int value = Integer.parseInt(String.valueOf(newValue));
        long millisValue = TimeUnit.SECONDS.toMillis(value);
        if (millisValue > Integer.MAX_VALUE) {
            throw new NumberFormatException("Timeout too large.");
        }
        return millisValue;
    }

...
public final class JasperPrintJobFactory {
    ...
    public static ResourcePrintJob createDashboardPrintJob(WebView webView, ResourceLookup resource) {
        ...
        ServerVersion serverVersion = ServerVersion.defaultParser().parse(serverInfoProvider.getServerVersion());

        String printName = resource.getLabel();
        if (resource.getResourceType() != ResourceLookup.ResourceType.legacyDashboard ||
                serverVersion.getVersionCode() >= ServerVersion.AMBER.getVersionCode()) {
